...
void callSubCtosql(int n, CDescRec* tmpDesc){
    ...
    switch(TESTDATA_MAP[n].srcType){
        ...
        case SQL_C_STINYINT:{
            ...
            memset(testP, 0,sizeof(char) * TESTDATA_MAP[n].targetLen);
            SQLLEN size;
            ConvertCToSQL(3,TESTDATA_MAP[n].srcType, &testSrc, sizeof(testSrc), testP, tmpDesc, false, iconv);
            commonSwitchctosql(TESTDATA_MAP[n].targetType, testP, n);    
            break;
        }
        case SQL_C_BIT:
        case SQL_C_UTINYINT:{
...
void commonSwitchctosql(int targetType, SQLPOINTER testP, int n){
    ...
    switch(targetType){
        ...
        case SQL_BIT:{
            char mtarget = *(char *)testP;
            char sqldata = stringToNum<int>(TESTDATA_MAP[n].target);
            EXPECT_FLOAT_EQ(sqldata, mtarget) <<TESTDATA_MAP[n].targetType  << TESTDATA_MAP[n].srcType ;
            delete((char *)testP);
            break; 
        }
        case SQL_TINYINT:{
        ...
        case SQL_SMALLINT:{
            short   mtarget = *(short *)testP;
            short  sqldata = stringToNum<short>(TESTDATA_MAP[n].target);
            EXPECT_FLOAT_EQ(sqldata, mtarget) <<TESTDATA_MAP[n].targetType  << TESTDATA_MAP[n].srcType ;
            delete((short *)testP);
            break; 
        ...
        case SQL_INTEGER:{
            ...
            int mtarget = *(int *)testP;
            int sqldata = stringToNum<int>(TESTDATA_MAP[n].target);
            EXPECT_FLOAT_EQ(sqldata, mtarget) <<TESTDATA_MAP[n].targetType  << TESTDATA_MAP[n].srcType ;
            delete((int *)testP);
            break; 
        }
        case SQL_BIGINT:{
            ...
            long long  mtarget = *(long long  *)testP;
            long long  sqldata = stringToNum<long long>(TESTDATA_MAP[n].target);
            EXPECT_FLOAT_EQ(sqldata, mtarget) <<TESTDATA_MAP[n].targetType  << TESTDATA_MAP[n].srcType ;
            delete((long long *)testP);
            break; 
        }
        ...
        case SQL_DOUBLE:{
            ...
            double mtarget = *(double *)testP;
            double sqldata = stringToNum<double>(TESTDATA_MAP[n].target);
            EXPECT_FLOAT_EQ(sqldata, mtarget) <<TESTDATA_MAP[n].targetType  << TESTDATA_MAP[n].srcType ;
            delete((double *)testP);
            break; 
        }
